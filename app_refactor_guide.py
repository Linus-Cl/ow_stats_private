"""
app_refactor_guide.py
=====================
ANNOTIERTE KOPIE von app.py — NUR als Migrations-Referenz!
→ Diese Datei NICHT ausführen. Die echte App bleibt in app.py.

Kommentar-Konventionen:
  # >>>MOVE TO: pfad/datei.py<<<   = dieser Block gehört dort hin
  # >>>KEEP HERE<<<                = bleibt in app.py (Bootstrap-Code)
  # >>>SPLIT<<<                    = aufteilen, Teil bleibt hier, Rest woanders
...
# >>>KEEP HERE<<< (oder in app.py lassen)
# utils/i18n.py, data/loader.py, api/routes.py werden später importiert


# >>>KEEP HERE<<< — App-Instanz bleibt immer in app.py
# --- App Initialization ---
app = Dash(
    __name__,
...
# >>>MOVE TO: api/routes.py<<< (Flask-Route GET /bye)
def bye():
    try:
...
# >>>MOVE TO: utils/sessions.py<<< (mit bestehender sessions.py zusammenführen!)
# --- Active sessions (live online counter) ---


# >>>MOVE TO: utils/sessions.py<<<
def _init_active_db():
    try:
        conn = sqlite3.connect(ACTIVE_DB)
        cur = conn.cursor()
...
# >>>MOVE TO: utils/i18n.py<<< (bestehende i18n.py prüfen!)
# --- i18n helper ---
...
# >>>KEEP HERE<<< (oder utils/sessions.py) — Self-Ping läuft beim App-Start
...
# >>>MOVE TO: utils/assets.py<<< (mit assets_utils.py zusammenführen!)
...
# >>>KEEP HERE<<< — Layout bleibt in app.py (Shell mit Tabs/Navbar)
# Nur Tab-INHALTE auslagern nach pages/*.py
# --- Layout ---
app.layout = html.Div(
    [
        dcc.Location(id="url"),
...
# >>>MOVE TO: api/routes.py<<< (Flask-Route GET /input)
@server.route("/input")
def input_page():
...
# >>>KEEP HERE<<< — Theme-Callbacks bleiben in app.py (global, nicht Tab-spezifisch)
@app.callback(
...
# >>>KEEP HERE<<< — Language-Toggle bleibt in app.py (global)
...
# >>>MOVE TO: pages/stats.py<<< — Stats-Callbacks
# --- Callbacks ---
@app.callback(
...
# >>>MOVE TO: pages/stats.py<<< — HAUPTGRAPH-CALLBACK (größter Callback)
@app.callback(
...
# >>>MOVE TO: utils/sessions.py<<< (Heartbeat-Callbacks)
...
# >>>KEEP HERE<<< — Startup-Sequenz bleibt in app.py
...
# >>>KEEP HERE<<< — Entry point bleibt in app.py
